...
bool Stegano::DataChecks()
{
	...
	DPRINT("Image Pixel Width", Stegano::DIBHeader.get()->bV5Width);
	DPRINT("Image Pixel Height", Stegano::DIBHeader.get()->bV5Height);
	DPRINT("Size Of Image", Stegano::fileHeader.get()->bfSize)
	DPRINT("Offset To BitMap", Stegano::bitMapOffset)


	if (Stegano::DIBHeader.get()->bV5Width % 4 == 0)
	{
	...
	{
		Stegano::szRow = (Stegano::DIBHeader.get()->bV5Width * BIT_24) / 0x08;
		Stegano::padding = Stegano::PaddingCalc(0x04);	
		DPRINT("Padding Per Row", Stegano::padding)
		Stegano::imageIsPadded = true;
	}


	Stegano::szBitMap = (Stegano::DIBHeader.get()->bV5Width * Stegano::DIBHeader.get()->bV5Height * BIT_24 / 0x08);
...
bool Stegano::ReadPaddedBitMap()
{
	...
	for (std::size_t row = 0; row < Stegano::DIBHeader.get()->bV5Height; row++)
	{
			
		Stegano::readBmpFile.get()->read((PCHAR)(Stegano::imageBuffer.data() + Stegano::szRow * row), Stegano::szRow);
		if (Stegano::readBmpFile.get()->fail())
		{
			EPRINT("Error While Reading BitMap")
			return false;
...
bool Stegano::WriteNonPaddedBitMap()
{
	...
	Stegano::writeBmpFile.get()->write((LPCSTR)Stegano::imageBuffer.data(), imageBuffer.size());
	if (Stegano::writeBmpFile.get()->fail())
	{
...
bool Stegano::WritePaddedBitMap()
{
	...
	for (std::size_t row = 0; row < Stegano::DIBHeader.get()->bV5Height; row++)
	{
		...
		Stegano::writeBmpFile.get()->write((LPCSTR)(Stegano::imageBuffer.data() + Stegano::szRow * row), Stegano::szRow);
...
__forceinline std::size_t Stegano::PaddingCalc(__in std::size_t alignStride) const
{
	std::size_t newSzRow = Stegano::szRow;
	while (newSzRow % alignStride != 0)
	{
		newSzRow++;
	}

	return newSzRow - Stegano::szRow;
}