...
private:
	std::string fileName;
	std::unique_ptr<const BITMAPFILEHEADER> fileHeader;
	std::unique_ptr<const BITMAPV5HEADER> DIBHeader;
...
private:
	...
	bool DataChecks();
	__forceinline std::size_t PaddingCalc(__in std::size_t alignStride) const;
